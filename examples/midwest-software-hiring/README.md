...
The question was: which venture-backed software companies in the Midwest (IL,
IN, IA, MI, MN, MO, OH, WI) are growing right now? For anyone selling to
startups, or looking for a job at one, a recent funding round and open roles
are two useful timing signals.
...
- This run stops at the filter (`judge.limit: 0`), so the list is ranked by
  signals rather than by fit for a particular seller. Add context files and
